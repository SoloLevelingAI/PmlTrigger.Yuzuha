...
`mcp__YuzuhaToolkit__run_pml_command_list`, and
...
Set `YUZUHA_PML_PIPE` in both processes to use an instance-specific pipe;
the default remains `yuzuha.pml.command.v1`.

## Tool: get_connection_status

Returns the cached PlantHost heartbeat state without executing PML. Important
fields are `State`, `IsConnected`, `LastSuccessUtc`,
`LastLatencyMilliseconds`, `ConsecutiveFailures`, `LastError`,
`HeartbeatRunning`, and `HeartbeatLoopError`. State transitions are
`Connecting -> Connected` on success, or
`Connecting -> Degraded -> Disconnected` after repeated failures.
...
|---|---|---|---|
| `pmlCommand` | string | — | Expression that returns an ARRAY, for example `!!YuzuhaReadCurrentElement(30,3,'member')`. |
...
Pass only the array-returning expression. Do not pass a global assignment such
as `!!TEMP = ...` or an element assignment such as `!!TEMP[1] = ...`; the host
already performs the assignment and `SIZE()` check. Use the public
`YuzuhaReadCurrentElement`, `YuzuhaReadDbref`, or `YuzuhaReadGlobal` entry
points.
...
- `$P <path>` prints text and is not a reliable CE navigation command. Navigate
  through the site's deployed command wrapper and then reread CE to verify it.
